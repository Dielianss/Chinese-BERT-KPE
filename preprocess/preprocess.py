...
dataset_dict = {'openkp': [('EvalPublic', 'eval'), ('Dev', 'dev'), ('Train', 'train')],
                'kp20k': [('training', 'train'), ('dev', 'dev'), ('eval', 'eval')]}
...
# -------------------------------------------------------------------------------------
# -------------------------------------------------------------------------------------
# filter absent keyphrases. absent keyphrases: keyphrases that don't appear in source text
...
if __name__ == "__main__":
    # preprocess selector
    data_loader = {'openkp': openkp_loader, 'kp20k': kp20k_loader} # 加载源数据
    ...
    t0 = time.time()
    parser = argparse.ArgumentParser(description='preprocess_for_kpe.py', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    # add options
