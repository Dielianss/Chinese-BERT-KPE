...
def find_stem_answer(word_list, ans_list):
    """
    :param word_list:小写化后的 text 分词结果
    :param ans_list: keywords 分词结果
    :return:
    """
    # 标准化后的 text 分词结果(统一编码格式), 在标准化的基础上词干化的 text 分词结果
    norm_doc_char, stem_doc_char = norm_doc_to_char(word_list)
    # 标准化后的 keywords 分词结果(统一编码格式), 在标准化的基础上词干化的 keywords 分词结果, 返回 元组对 (norm, stem)
    ...
    for norm_ans_char, stem_ans_char in norm_stem_phrase_list:
        ...
        # 关键词必须在原文中出现, 否则即为 absent keywords, 舍去
...
def norm_doc_to_char(word_list):
    # Unicode 标准化: 确保所有字符串在底层有相同的表示, 某些字符有多种合法的编码表示, 通过标准化将其统一
    norm_char = unicodedata.normalize('NFD', " ".join(word_list))
    # stemmer: 将单词标准化, 即将单词词干化, 把单词替换为其词干(或词源)
