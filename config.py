...
def add_default_args(parser):
    ...
    modes.add_argument('--run_mode', type=str, choices=['train', 'eval'],
                       help='Select running mode. ')
    modes.add_argument('--dataset_class', type=str,
                       choices=["policy", "entertainment", "new_policy", "top4"],
    ...
    files.add_argument("--save_path", type=str, default='/data/yangj/keyphrase/results/',
...
def init_args_config(args):
    ...
    # Result folder
    folder_name = "%s_%s_%s_%s" % (args.run_mode, args.model_class, args.dataset_class,
    ...
    if args.load_checkpoint:
        checkpoint_path = os.path.join(args.save_path,
                                       "%s_%s_%s_%s" % ("train", args.model_class, args.dataset_class, args.pretrain_model_type.split('-')[0]),
                                       "checkpoints")
        tmp_epoch = -1
        ...
        if tmp_epoch == -1:
            ...
            exit()
        args.checkpoint_file = os.path.join(checkpoint_path, "%s.%s.%s.%s.%s" % (args.model_class, args.dataset_class, args.pretrain_model_type.split('-')[0], 'epoch_{}'.format(tmp_epoch), 'checkpoint'))
