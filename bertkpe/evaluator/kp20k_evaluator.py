...
def evaluate(candidates, references, urls):
    precision_scores, recall_scores, f1_scores = {1: [], 3: [], 5: [], 10: []}, \
                                                 {1: [], 3: [], 5: [], 10: []}, \
                                                 {1: [], 3: [], 5: [], 10: []}
    for url in urls:
        ...
        for l, j in zip(input_file, predicted_keywords):
            ...
            if len(match_list) < 5:
                for _ in range(5 - len(match_list)):
                    pred_keyphrases.append('')
                assert len(pred_keyphrases) == 5

            for topk in [1, 3, 5]:

                # Micro-Averaged  Method
                micropk = float(sum(match_list[:topk])) / float(len(pred_keyphrases[:topk])) if len(
                    pred_keyphrases[:topk]) > 0 else 0.0
                micrork = float(sum(match_list[:topk])) / float(len(true_keyphrases)) if len(true_keyphrases) > 0 else 0.0
